...
- **Themes / categories** (`THEMES`, `THEME_GROUPS`): five pool groups — the four sensory themes (`dingshu`/`spring`/`bamboo-water`/`night`) plus `food` (餐饮). Food is a category, not a sense: all `type: "food"` cards live under `theme: "food"` regardless of where they physically sit in `data/places.js` (grouping is by `theme`, not file position), so eating is decoupled from time-of-day. 夜晚 is now 住宿 / 自由 / 夜色 only. Each theme has a `--c-*` accent and a `[data-theme=…]` pool ambiance; the intro's "四种感官入口" still lists only the four senses (food is reachable via the 菜单 filter).
