...
- 主题（v1.8 起五个）：四种感官 dingshu / spring / bamboo-water / night，外加 food（餐饮）。food 是「类别」不是感官——所有 `type: "food"` 卡都归 `theme: "food"`（餐饮不分早晚，脱离夜晚），夜晚只剩住宿 / 自由 / 夜色。分组只看卡的 `theme`，与它在 places.js 里的物理位置无关（Tide/陶喜/不介 等散在各区但都是 food）。每个主题有 `--c-*` 卡片色和 `[data-theme=…]` 菜单区氛围色；简介的「四种感官入口」仍只列四感官，餐饮走菜单筛选进入。由 `[data-theme]` + CSS variables 驱动；点选卡片也会切到该卡主题。
